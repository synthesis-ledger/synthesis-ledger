...
async function run() {
    ...
    // Read and strip BOM if it exists
    ...
    // THE FULL RECIPE (Logic, BPS Matrix, Schemas, SLIs)
    ...
    const siloA = await streamGrok("grok-4-1-fast-non-reasoning", 
        "SRE Specialist: Audit for TOIL and operational waste in the provided Technical Specification and SRE Metrics.", context, "SILO A: TOIL");
    ...
    const siloC = await streamGrok("grok-4-1-fast-non-reasoning", 
        "Economist: Audit for incentive gaming and economic arbitrage in the BPS Aggregation and Path Score Computation.", context, "SILO C: ECONOMY");
    
    const siloD = await streamGrok("grok-4-1-fast-non-reasoning", 
        "Technical Architect: Audit for structural integrity and pattern compliance with the Genesis-Platinum lifecycle.", context, "SILO D: STRUCTURE");
